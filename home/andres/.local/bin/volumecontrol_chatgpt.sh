...
change_volume() {
    ...
    # Ensure target_step is not 0
    [ "$target_step" -eq 0 ] && target_step=1
    
    # Unmute if muted
    if pamixer $srce --get-mute | grep -q "true"; then
        pamixer $srce --unmute
    fi

    case $device in
        "pamixer")
            for ((i=0; i<target_step; i++)); do
                if $use_swayosd; then
                    swayosd-client "${mode}" "${delta}1"
                    exit 0
                elif command -v pamixer >/dev/null 2>&1; then
                    pamixer $srce -"$action" "1"
                else
                    if [ -z "$srce" ]; then
                        pactl set-sink-volume @DEFAULT_SINK@ "${delta}5%"
                    else
                        pactl set-source-volume @DEFAULT_SOURCE@ "${delta}5%"
                    fi
                fi
            done
            vol=$(pamixer $srce --get-volume 2>/dev/null || pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}' | tr -d '%')
            ;;
        "playerctl")
            local total_steps=$(echo "scale=2; $target_step / 0.01" | bc -l)
            playerctl --player="$srce" volume "$(awk -v step="$total_steps" 'BEGIN {print step}')${delta}"
...
toggle_mute() {
    ...
    case $device in
        "pamixer")
            ...
            ;;
        "playerctl")
            # Handle playerctl mute toggle (simplified)
            ...
            if [ "$(playerctl --player="$srce" volume | awk '{ printf "%.2f", $0 }')" != "0.00" ]; then
                ...
                if [ -f "$volume_file" ]; then
                    ...
                    playerctl --player="$srce" volume 0.1
                fi
            fi
            notify_mute
            ;;
...
toggle_output() {
    ...
    # Handle case when no valid sink is found
    if [ -z "$current_index" ]; then
        notify-send -t 2000 -r 2 -u critical "No valid output devices found"
        exit 1
    fi
